static char rcsid[] = "$Id: tableint.c 224367 2021-07-07 19:23:12Z twu $";
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
static int
value_cmp (const void *x, const void *y) {
  struct binding *a = * (struct binding **) x;
  struct binding *b = * (struct binding **) y;
...
void **
Tableint_keys_by_value (T table, void *end) {
  void **keyarray;
  int i, j = 0;
  struct binding **buckets, *p;
  ...
  qsort(buckets,table->length,sizeof(struct binding *),value_cmp);
