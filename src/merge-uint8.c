static char rcsid[] = "$Id: aa2c4d583b43a268ee47177930790cefdb61ba6d $";
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
merge_8x8 (__m512i *__restrict__ vMergedA, __m512i *__restrict__ vMergedB, __m512i vA, __m512i vB) {
  ...
  vTmp = _mm512_alignr_epi64(vMin, vMin, 1); /* Rotate Min by 64 bits */
  vMin = _mm512_min_epu64(vTmp, vMax);
  /* print_vector_512(vTmp,"Tmp 8"); */
  /* print_vector_512(vMin,"Min 8"); */
...
Merge_uint8 (UINT8 *__restrict__ dest, UINT8 *__restrict__ A,
  ...
  if ((nC = nA + nB) == 0) {
    ...
    MALLOC_ALIGN(C,nC * sizeof(UINT8));
    C0 = C;
  }

  if (nA == 0) {
    memcpy(C,B,nB * sizeof(UINT8));
    return C0;
  } else if (nB == 0) {
    memcpy(C,A,nA * sizeof(UINT8));
    return C0;
  } else {
    Aend = &(A[nA]);
    Bend = &(B[nB]);
  ...
  if (A < Aend - 16 && B < Bend - 16) {
    ...
    if ((nextA = A[16]) < (nextB = B[16])) {
      vOld512_0 = _mm512_load_si512((__m512i *) B); B += 8;
      vOld512_1 = _mm512_load_si512((__m512i *) B); B += 8;
      vNew512_0 = _mm512_load_si512((__m512i *) A); A += 8;
      vNew512_1 = _mm512_load_si512((__m512i *) A); A += 8;
    } else {
      vOld512_0 = _mm512_load_si512((__m512i *) A); A += 8;
      vOld512_1 = _mm512_load_si512((__m512i *) A); A += 8;
      vNew512_0 = _mm512_load_si512((__m512i *) B); B += 8;
      vNew512_1 = _mm512_load_si512((__m512i *) B); B += 8;
    }
    merge_16x16_network(&vMerged512_0,&vMerged512_1,&vOld512_0,&vOld512_1,
			vOld512_0,vOld512_1,vNew512_0,vNew512_1);

    _mm512_stream_si512((__m512i *) C,vMerged512_0); C += 8;
    _mm512_stream_si512((__m512i *) C,vMerged512_1); C += 8;
    ...
    if (nextA < nextB) {
      B -= 8; _mm512_store_si512((__m512i *) B,vOld512_1);
      B -= 8; _mm512_store_si512((__m512i *) B,vOld512_0);
    } else {
      A -= 8; _mm512_store_si512((__m512i *) A,vOld512_1);
      A -= 8; _mm512_store_si512((__m512i *) A,vOld512_0);
    }
  }
#endif
  ...
  /* Final elements to add from A or B.  Note that A == Aend or B == Bend or both  */
