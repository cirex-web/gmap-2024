static char rcsid[] = "$Id: sedgesort.c 225220 2022-11-01 22:46:01Z twu $";
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
#include <stdlib.h>
#endif
#include <limits.h>		/* For INT_MAX and UINT64_MAX */
...
static void
insertion_sort_order_uint4 (register int order[], register unsigned int array[], register int len) {
  register int i, j;
  register int temp;
  /* register unsigned int *hi, *lo; */
...
static void
insertion_sort_order_int2 (register int order[], register int array1[], register int array2[], register int len) {
  register int i, j;
  register int temp;
  /* register int *hi, *lo; */
  register int temporary;
  ...
  j = 0;
  for (i = 1; i < thresh; i++) {
    if (array1[order[i]] + array2[order[i]] < array1[order[j]] + array2[order[j]]) {
      j = i;
    }
  }
  SWAP(order[j],order[0]);
  ...
  for (i = 1; i < len; i++) {
    j = i;
    temp = order[j];
    temporary = array1[temp] + array2[temp];
    while (array1[order[j-1]] + array2[order[j-1]] > temporary) {
      order[j] = order[j-1];
      j--;
    }
    order[j] = temp;
...
/* Used by Knownsplicing to call EF64_new */
#ifdef HAVE_64_BIT
static void
insertion_sort_order_uint8 (register int order[], register UINT8 array[], register int len) {
  register int i, j;
  register int temp;
  /* register UINT8 *hi, *lo; */
  register UINT8 temporary;
  int thresh;
...
static void
partial_quickersort_order_uint4 (register int order[], register unsigned int array[],
  ...
  if (upper - lower > CUTOFF) {
    ...
    partial_quickersort_order_uint4(order, array, lower, j-1);
    partial_quickersort_order_uint4(order, array, i, upper);
  }
...
static void
partial_quickersort_order_int2 (register int order[], register int array1[], register int array2[],
			       register int lower, register int upper) {
  register int i, j;
  register int temp;
  register int pivot;
  ...
  if (upper - lower > CUTOFF) {
    ...
    i = lower;
    j = upper + 1;
    pivot = array1[order[lower]] + array2[order[lower]];

    while (1) {
      do i++; while (array1[order[i]] + array2[order[i]] < pivot);
      do j--; while (array1[order[j]] + array2[order[j]] > pivot);
      if (j < i) break;
      SWAP(order[i], order[j]);
    }

    SWAP(order[lower], order[j]);
    partial_quickersort_order_int2(order, array1, array2, lower, j-1);
    partial_quickersort_order_int2(order, array1, array2, i, upper);
  }
...
#ifdef HAVE_64_BIT
static void
partial_quickersort_order_uint8 (register int order[], register UINT8 array[],
				 register int lower, register int upper) {
  register int i, j;
  register int temp;
  register UINT8 pivot;
  ...
  if (upper - lower > CUTOFF) {
    ...
    SWAP(order[lower], order[j]);
    partial_quickersort_order_uint8(order, array, lower, j-1);
    partial_quickersort_order_uint8(order, array, i, upper);
  }
...
Sedgesort_order_uint4 (register unsigned int array[], register int len) {
  ...
  partial_quickersort_order_uint4(order, array, 0, len-1);
  insertion_sort_order_uint4(order, array, len);
  return order;
}
...
Sedgesort_order_int2 (register int array1[], register int array2[], register int len) {
  ...
  array1[len] = INT_MAX/2;
  array2[len] = INT_MAX/2;

  partial_quickersort_order_int2(order, array1, array2, 0, len-1);
  insertion_sort_order_int2(order, array1, array2, len);
  return order;
}
...
Sedgesort_order_uint8 (register UINT8 array[], register int len) {
  ...
  array[len] = UINT64_MAX;

  partial_quickersort_order_uint8(order, array, 0, len-1);
  insertion_sort_order_uint8(order, array, len);
  return order;
}
#endif
...
main (int argc, char *argv[]) {
  ...
  order = Sedgesort_order_uint4(array,10);
