static char rcsid[] = "$Id: e834c354b1fb5bb12e3f814268fdb9938c0205eb $";
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
Univ_IIT_interval_bounds_transcriptome (Trcoord_T *low, Trcoord_T *high, Chrpos_T *length, T this,
  ...
  interval = &(this->intervals[index-1]);
  *low = (Trcoord_T) Univinterval_low(interval);
  *length = Univinterval_length(interval);
  *high = (Trcoord_T) Univinterval_high(interval) + 1;
  return;
}
...
Univ_IIT_update_chrnum (int *chrnum, Univcoord_T *chroffset, Univcoord_T chrhigh, Chrpos_T *chrlength,
  ...
  /* Need to call Univ_IIT_intervals_setup at start of program */
  Univcoord_T goal;
...
#ifndef FAST_CHR_UPDATE
    *chrnum = Univ_IIT_get_one(this,low,low);
    Univ_IIT_interval_bounds(&(*chroffset),&chrhigh,&(*chrlength),this,*chrnum,circular_typeint);

#else
    /* Code from segment-search.c */
    j = 1;
    goal = low + 1;
    ...
    debug4(printf("Got chrnum %d, chroffset %u, chrhigh %u\n",chrnum,chroffset,chrhigh));
#endif
  ...
  if (high > chrhigh) {
	...
	*chrnum = test_chrnum; *chroffset = test_chroffset; chrhigh = test_chrhigh, *chrlength = test_chrlength;
    ...
    /* Code from segment-search.c */
    endj = 0;
    while (endj+1 < nchromosomes_local && high > chrhighs_local[endj+1]) {
      debug1(printf("For high %u, advancing to chrhigh %u\n",high,chrhighs_local[endj+1]));
      endj++;
    }
    if (endj+1 < nchromosomes_local) {
      endj++;
    }
    debug4(printf("endj is %d\n",endj));
    ...
    if ((/*inbounds = */high - test_chroffset) > most_inbounds) {
...
#else
    /* Code from segment-search.c */
    if ((/*inbounds = */high - chroffsets[(chrnum+endj)-1]) > most_inbounds) {
      j = endj;
    }
...
Univ_IIT_get_trnum (Trcoord_T *troffset, Trcoord_T *trhigh, Trcoord_T *trlength,
  ...
  Trnum_T trnum, start_trnum, end_trnum, test_trnum;
  Trcoord_T test_troffset, test_trhigh;
  Trcoord_T most_inbounds, inbounds, test_trlength;

  assert(low < high);

  trnum = (Trnum_T) Univ_IIT_get_one(this,low,low);
  Univ_IIT_interval_bounds_transcriptome(&(*troffset),&(*trhigh),&(*trlength),this,trnum);
  debug4(printf("Setting trnum to be %d\n",trnum));

  if (high > *trhigh) {
    /* Straddles two or more transcripts */
    /* Test first transcript */
    start_trnum = trnum;
    most_inbounds = (*trhigh) - low;
    debug4(printf("Straddle.  First transcript inbounds: %u.  ",most_inbounds));

    
    /* Test middle transcripts */
    end_trnum = Univ_IIT_get_one(this,high-1U,high-1U);
    for (test_trnum = start_trnum + 1; test_trnum < end_trnum; test_trnum++) {
      Univ_IIT_interval_bounds_transcriptome(&test_troffset,&test_trhigh,&test_trlength,this,test_trnum);
      debug4(printf("Next inbounds: %u.  ",test_trhigh - test_troffset));
      if ((inbounds = test_trhigh - test_troffset) > most_inbounds) {
	trnum = test_trnum; *troffset = test_troffset; *trhigh = test_trhigh, *trlength = test_trlength;
	most_inbounds = inbounds;
      }
    }

    /* Test last transcript */
    Univ_IIT_interval_bounds_transcriptome(&test_troffset,&test_trhigh,&test_trlength,this,end_trnum);
    debug4(printf("Last inbounds: %u\n",high - test_troffset));
    if ((inbounds = high - test_troffset) > most_inbounds) {
      trnum = end_trnum; *troffset = test_troffset; *trhigh = test_trhigh, *trlength = test_trlength;
      /* most_inbounds = inbounds; */
    }
  }

  debug4(printf("Returning trnum %d\n",trnum));
  return trnum;
}
