static char rcsid[] = "$Id: ce81f2fe5aca13fd7d98e2b377ec849f0f501f76 $";
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
#ifndef HAVE_MEMCPY
...
#if 0
static char *fwd_conversion = "ACGT";
static char *rev_conversion = "ACGT";
#endif
...
Genome_free (T *old) {
  if (*old) {
    if ((*old)->accession != NULL) {
      FREE((*old)->accession);
    }

    if ((*old)->access == ALLOCATED_PRIVATE) {
      FREE_KEEP((*old)->blocks);
...
Genome_new (char *genomesubdir, char *fileroot, char *alt_suffix,
  ...
  new->accession = (char *) NULL;			   /* Used only by Genome_from_sequence */

  if (alt_suffix != NULL) {
    if (revcompp == true) {
      filename = (char *) CALLOC(strlen(genomesubdir)+strlen("/")+strlen(fileroot)+
				 strlen(".genomecomp.rev.")+strlen(alt_suffix)+1,sizeof(char));
      sprintf(filename,"%s/%s.genomecomp.rev.%s",genomesubdir,fileroot,alt_suffix);
    } else {
      filename = (char *) CALLOC(strlen(genomesubdir)+strlen("/")+strlen(fileroot)+
				 strlen(".genomecomp.")+strlen(alt_suffix)+1,sizeof(char));
      sprintf(filename,"%s/%s.genomecomp.%s",genomesubdir,fileroot,alt_suffix);
    }
  } else {
    if (revcompp == true) {
      filename = (char *) CALLOC(strlen(genomesubdir)+strlen("/")+strlen(fileroot)+
				 strlen(".genomecomp.rev")+1,sizeof(char));
      sprintf(filename,"%s/%s.genomecomp.rev",genomesubdir,fileroot);
    } else {
      filename = (char *) CALLOC(strlen(genomesubdir)+strlen("/")+strlen(fileroot)+
				 strlen(".genomecomp")+1,sizeof(char));
      sprintf(filename,"%s/%s.genomecomp",genomesubdir,fileroot);
  ...
  if (access == USE_ALLOCATE) {
#ifndef UTILITYP
    fprintf(stderr,"Allocating memory for compressed genome (oligos)...");
#endif
    if (sharedp == true) {
...
#ifndef UTILITYP
    fprintf(stderr,"Pre-loading compressed genome (oligos)...");
#endif
    ...
    new->blocks = (Genomecomp_T *) Access_mmap(&new->fd,&new->len,&seconds,filename,/*randomp*/false);
    if (new->blocks == NULL) {
...
Genome_from_sequence (Sequence_T sequence) {
  ...
  new->genomelength = Sequence_fulllength(sequence);
  new->blocks = Compress_create_blocks_comp(Sequence_fullpointer(sequence),Sequence_fulllength(sequence));

  if ((accession = Sequence_accession(sequence)) == NULL) {
    new->accession = (char *) NULL;
  } else {
    new->accession = (char *) MALLOC((strlen(accession)+1)*sizeof(char));
    strcpy(new->accession,accession);
  }
...
#if 0
/*                                  T  G  C  A  N/X */
static unsigned char complInt[5] = {3, 2, 1, 0, 4};

static void
make_complement_inplace_int (unsigned char *intstring, Chrpos_T length) {
  char temp;
  unsigned int i, j;

  for (i = 0, j = length-1; i < length/2; i++, j--) {
    temp = complInt[(int) intstring[i]];
    intstring[i] = complInt[(int) intstring[j]];
    intstring[j] = temp;
  }
  if (i == j) {
    intstring[i] = complInt[(int) intstring[i]];
  }
...
/* Also in genome_blocks.c for GSNAP */
static void
uncompress_mmap (char *gbuffer1, Genomecomp_T *blocks, Univcoord_T startpos, 
		 Univcoord_T endpos) {
  /* Chrpos_T length = endpos - startpos; */
  Univcoord_T startblock, endblock, ptr;
  Genomecomp_T high, low, flags;
  ...
  if (endblock == startblock) {
    ...
    debug3(printf("low0:  %02X => %s\n",low & 0x0000FFFF,nucleotides[low & 0x0000FFFF]));
    debug3(printf("low0:  %02X => %s\n",low >> 16,nucleotides[low >> 16]));
    debug3(printf("high0: %02X => %s\n",high & 0x0000FFFF,nucleotides[high & 0x0000FFFF]));
    debug3(printf("high1: %02X => %s\n",high >> 16,nucleotides[high >> 16]));
...
Genome_dump (T genome, Univcoord_T startpos, Univcoord_T endpos, int wraplength) {
  /* Chrpos_T length = endpos - startpos; */
  Filestring_T fp;
  Genomecomp_T *blocks = genome->blocks;
  Univcoord_T startblock, endblock, ptr;
  Genomecomp_T high, low, flags;
  char Buffer[32];
  Chrpos_T pos = 0, p;		/* Could potentially print an entire chromosome */
  int startdiscard, enddiscard, k, i;
  int wrappos = 0, n;
  int blocki = 0;
  ...
  if (endblock == startblock) {
    ...
    memcpy(&(Buffer[0]),nucleotides[low & 0x0000FFFF],8);
    memcpy(&(Buffer[8]),nucleotides[low >> 16],8);
    memcpy(&(Buffer[16]),nucleotides[high & 0x0000FFFF],8);
    memcpy(&(Buffer[24]),nucleotides[high >> 16],8);
    if (flags) {
    ...
    k = startdiscard;
    p = pos;
    while (pos + (enddiscard - startdiscard) - p >= (Chrpos_T) (n = wraplength - wrappos)) {
      printf("%.*s\n",n,&(Buffer[k]));
      p += n; wrappos = 0; k += n;
    }
    if ((n = pos + (enddiscard - startdiscard) - p) > 0) {
      printf("%.*s\n",n,&(Buffer[k]));
      /* p += n; wrappos += n; k += n; */
    }
    /* pos = p; */
    ...
    k = startdiscard;
    p = pos;
    while (pos + (32 - startdiscard) - p >= (Chrpos_T) (n = wraplength - wrappos)) {
      printf("%.*s\n",n,&(Buffer[k]));
      p += n; wrappos = 0; k += n;
    }
    if ((n = pos + (32 - startdiscard) - p) > 0) {
      printf("%.*s",n,&(Buffer[k]));
      p += n; wrappos += n; k += n;
    }
    pos = p;
	...
	fp = Filestring_new();
      ...
      k = 0;
      p = pos;
      while (pos + 32 - p >= (Chrpos_T) (n = wraplength - wrappos)) {
	FPRINTF(fp,"%.*s\n",n,&(Buffer[k]));
	p += n; wrappos = 0; k += n;
      }
      if ((n = pos + 32 - p) > 0) {
	FPRINTF(fp,"%.*s",n,&(Buffer[k]));
	p += n; wrappos += n; /* k += n; */
      }
      pos = p;
	...
	for (i = 0; i < 32; i++) {
      ...
      k = 0;
      p = pos;
      while (pos + enddiscard - p >= (Chrpos_T) (n = wraplength - wrappos)) {
	printf("%.*s\n",n,&(Buffer[k]));
	p += n; wrappos = 0; k += n;
      }
      if ((n = pos + enddiscard - p) > 0) {
	printf("%.*s",n,&(Buffer[k]));
	p += n; wrappos += n; /* k += n; */
      }
      /* pos = p; */
...
static void
uncompress_mmap_snps_subst (char *gbuffer1,
#if !defined(GSNAP)
  ...
  if (endblock == startblock) {
    /* Special case */
#ifdef WORDS_BIGENDIAN
    althigh = Bigendian_convert_uint(genomealt->blocks[ptr]);
    altlow = Bigendian_convert_uint(genomealt->blocks[ptr+1]);
    refflags = Bigendian_convert_uint(genome->blocks[ptr+2]);
#else
    althigh = genomealt->blocks[ptr]; altlow = genomealt->blocks[ptr+1]; refflags = genome->blocks[ptr+2];
#endif
...
#if 0
static bool
uncompress_mmap_nucleotides_wstatus (unsigned char *intstring, Genomecomp_T *blocks, Univcoord_T startpos, 
				     Univcoord_T endpos) {
  /* Chrpos_T length = endpos - startpos; */
  ...
  if (endblock == startblock) {
	...
	*intstring++ = (unsigned char) (low & 0x03);
#ifdef DEBUG3
	if (check_nucleotide(intstring[k-1],gbuffer_debug[k-1])) {
	...
	*intstring++ = (unsigned char) (high & 0x03);
#ifdef DEBUG3
	if (check_nucleotide(intstring[k-1],gbuffer_debug[k-1])) {
...
#if 0
static void
uncompress_mmap_nucleotides (unsigned char *intstring, Genomecomp_T *blocks, Univcoord_T startpos, 
			     Univcoord_T endpos) {
  /* Chrpos_T length = endpos - startpos; */
  Univcoord_T startblock, endblock, ptr;
  Genomecomp_T high, low;
...
static void
uncompress_mmap_int_string (unsigned char *intstring, Genomecomp_T *blocks, Univcoord_T startpos, 
			    Univcoord_T endpos) {
  /* Chrpos_T length = endpos - startpos; */
  Univcoord_T startblock, endblock, ptr;
  Genomecomp_T high, low, flags;
...
#ifdef UTILITYP
static void
uncompress_mmap_int_string_convert (unsigned char *intstring, Genomecomp_T *blocks, Univcoord_T startpos, 
  ...
  if (endblock == startblock) {
	...
	if (flags & 0x01) {
	  ...
	  *intstring++ = conversion[(int) (low & 0x03)];
	}
	low >>= 2;
	flags >>= 1;
	startdiscard++;
	...
	if (flags & 0x01) {
	  ...
	  *intstring++ = conversion[(int) (high & 0x03)];
	}
	high >>= 2;
	flags >>= 1;
	startdiscard++;
...
#if 0
void
Genome_user_setup (Genomecomp_T *genome_blocks_in, Univcoord_T genomelength_in) {
...
Genome_fill_buffer (
  ...
  univdiagonal = left + length;
  
  /* TRIM QUERY AT GENOME BOUNDS */
  pos5 = (univdiagonal >= 0 + (Univcoord_T) length) ? 0 : (int) (0 - left);
  pos3 = (univdiagonal <= genome->genomelength) ? (int) length : (int) (genome->genomelength - left);
  assert(pos5 < pos3);

  uncompress_mmap(&(gbuffer1[pos5]),genome->blocks,left+pos5,left+pos3);

  for (i = 0; i < pos5; i++) {
    gbuffer1[i] = '*';
  }
  for (i = pos3; i < (int) length; i++) {
    gbuffer1[i] = '*';
  }
  gbuffer1[length] = '\0';

  debug(printf("Got sequence at %llu with length %u, forward, %d..%d %s\n",
	       (unsigned long long) left,length,pos5,pos3,gbuffer1));
...
Genome_fill_buffer_blocks (Univcoord_T left, Chrpos_T length, char *gbuffer1) {
  ...
  if (length > 0) {
    assert(left + length >= left);
#ifdef EXTRACT_GENOMICSEG
    uncompress_mmap_bitbybit(gbuffer1,genome->blocks,left,left+length,DEFAULT_CHARS,A_FLAGS);
#else
    uncompress_mmap(gbuffer1,genome->blocks,left,left+length);
#endif
  }
  gbuffer1[length] = '\0';
  return;
...
#if 0
void
Genome_fill_buffer_bounds (Univcoord_T left, int pos5, int pos3, char *gbuffer1) {
  uncompress_mmap(&(gbuffer1[pos5]),genome->blocks,left+pos5,left+pos3);
  gbuffer1[pos3] = '\0';
  return;
}
...
#if !defined(GSNAP) && !defined(UTILITYP)
void
Genome_fill_buffer_blocks_noterm (T genome, T genomealt, Univcoord_T left, Chrpos_T length,
				  char *gbuffer1, char *gbuffer2) {
  
  if (length > 0) {
    assert(left + length >= left);
    uncompress_mmap(gbuffer1,genome->blocks,left,left+length);
    uncompress_mmap_snps_subst(gbuffer2,genome,genomealt,left,left+length);
  }
  /* gbuffer1[length] = '\0'; */
  return;
}
...
Genome_fill_buffer_alt (
  ...
  uncompress_mmap_snps_subst(gbuffer1,
#if !defined(GSNAP)
			     genome,genomealt,
#endif
			     left,left+length);
...
Genome_fill_buffer_int_string (T this, Univcoord_T left, Chrpos_T length, unsigned char *intstring,
  ...
  if (conversion == NULL) {
    uncompress_mmap_int_string(intstring,this->blocks,left,left+length);
  } else {
    uncompress_mmap_int_string_convert(intstring,this->blocks,left,left+length,conversion);
  }

  intstring[length] = 0xFF;
...
Genome_get_char (char *charalt,
  ...
  if ((c = uncompress_one_char(genome->blocks,left,/*flagchar*/'N',CHARTABLE)) == 'N') {
    *charalt = c;
  } else {
    *charalt = uncompress_one_char_ignore_flags(genomealt->blocks,left);
  }
...
Genome_get_segment_right (char *segment, char *segmentalt, T genome, T genomealt,
  ...
  uncompress_mmap(segment,genome->blocks,left,left+length-out_of_bounds);
  segment[length] = '\0';
  if (revcomp == true) {
    make_complement_inplace(segment,length);
  }

  if (genomealt->blocks == genome->blocks) {
    ...
    uncompress_mmap_snps_subst(segmentalt,genome,genomealt,left,left+length-out_of_bounds);
    segmentalt[length] = '\0';
    if (revcomp == true) {
...
Genome_get_segment_left (char *segment, char *segmentalt, T genome, T genomealt,
			 Univcoord_T right, Chrpos_T length, Univcoord_T chroffset, bool revcomp) {
  Chrpos_T out_of_bounds, i;
  ...
  uncompress_mmap(&(segment[out_of_bounds]),genome->blocks,right-length+out_of_bounds,right);
  segment[length] = '\0';
  if (revcomp == true) {
    make_complement_inplace(segment,length);
  }
  ...
  if (genomealt->blocks == genome->blocks) {
    ...
    uncompress_mmap_snps_subst(&(segmentalt[out_of_bounds]),genome,genomealt,right-length+out_of_bounds,right);
    segmentalt[length] = '\0';
    if (revcomp == true) {
      make_complement_inplace(segmentalt,length);
    }
