static char rcsid[] = "$Id: 39fa5e620ccaa8b4de25c3a3b7c594bf59e8b11e $";
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
unsigned int *
Uintlist_to_array (T list, unsigned int end) {
  unsigned int *array;
  int n, i;

  n = Uintlist_length(list);
  array = (unsigned int *) MALLOC((n+1)*sizeof(unsigned int));
  for (i = 0; i < n; i++) {
    array[i] = list->first;
    list = list->rest;
  }
...
#ifdef HAVE_64_BIT
UINT8 *
Uintlist_to_uint8_array (T list, UINT8 end) {
  UINT8 *array;
  int n, i;
...
unsigned int *
Uintlist_to_array_n (int *n, T list) {
  unsigned int *array;
  int i;
